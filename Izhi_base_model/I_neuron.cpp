...
I_neuron::I_neuron () {
    a = 0.03;    b = -2.0;    c = -50.0;    d = 100;
    threshold = 35.0;
    num_steps=1000;
    tau = 0.1; vt = -40;
    k = 0.7;  vr = -60.0;
    C=1.0;
    v = new double [num_steps];//1000 time steps as default
    u = new double [num_steps];
    v[0]=vr;
    u[0]=0.0;
}
I_neuron::I_neuron (double a_new, double b_new, double c_new, double d_new, double v_peak, int steps){
    a = a_new;  b = b_new; c = c_new;
    d = d_new; threshold = v_peak;
    tau = 0.1; vt = -40;
    k = 0.7;  vr = -60.0;
    C=1.0;
    num_steps = steps;
...
void I_neuron::update (int step, double I){
    ...
    v[step+1] = v[step]+tau*(k*(v[step]-vr)*(v[step]-vt)-u[step]+I)/C;
    u[step+1]= u[step] +tau*a*(b*(v[step]-vr)-u[step]);
    if(v[step+1]>=threshold){
        v[step]=threshold;
        v[step+1]=c;
        u[step+1]+=d;
    }
